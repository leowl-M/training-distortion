...
const EFFECTS_CATEGORIES = {
  ...
  water: { label: "Acqua & Onde", items: {
    ...
    ripple_box: { name: 'Ripple Rettangolare' }, caustic_light: { name: 'Luce Caustica' }
  }},
  space: { label: "Spazio & Geometria", items: {
    ...
    polar_warp: { name: 'Warp Polare' }, rose_wave: { name: 'Curva Rosa' }, lissajous: { name: 'Lissajous' }
  }},
  ...
  retro: { label: "Retro & Glitch", items: {
    ...
    interlace: { name: 'Interlacciato TV' }, data_mosher: { name: 'Data Moshing' }
  }},
  mirror: { label: "Simmetrie", items: {
    ...
    mirror_diag: { name: 'Specchio Diagonale' }
  }},
  optics: { label: "Ottica & Cinematica", items: {
    prism: { name: 'Dispersione Prisma' }, chromatic_ring: { name: 'Anello Cromatico' }, anamorphic: { name: 'Squeeze Anamorfic' }, zoom_blur: { name: 'Zoom Mosso' }, light_streak: { name: 'Scia Luminosa' }
  }},
  deform: { label: "Deformazioni", items: {
    squeeze_x: { name: 'Schiacciamento X' }, squeeze_y: { name: 'Schiacciamento Y' }, shockwave: { name: "Onda d'Urto" }, pendulum: { name: 'Pendolo' }, wormhole: { name: 'Wormhole' }, earthquake: { name: 'Terremoto' }
  }},
  specials: { label: "Speciali", items: {
    melt: { name: 'Scioglimento (Melt)' }, heartbeat: { name: 'Battito Cardiaco' },
    tentacle: { name: 'Tentacolo' }, smoke: { name: 'Fumo Ascendente' }, coral: { name: 'Corallo Frattale' }
  }}
};
...
const EFFECT_EXTRA_DEFS = {
  ...
  shockwave: [
    { id: 'sw_speed', label: 'Velocità espansione', min: 20, max: 200, step: 1, default: 100 }
  ],
  wormhole: [
    { id: 'wh_tight', label: 'Compressione centrale', min: 20, max: 150, step: 1, default: 60 }
  ],
  polar_warp: [
    { id: 'pw_segs', label: 'Segmenti petalo', min: 1, max: 8, step: 1, default: 3 }
  ],
  anamorphic: [
    { id: 'an_ratio', label: 'Rapporto d\'aspetto', min: 30, max: 200, step: 1, default: 100 }
  ],
  tentacle: [
    { id: 'ten_arms', label: 'Numero braccia', min: 1, max: 6, step: 1, default: 2 }
  ]
};
...
function applyDistortion(srcData, width, height, t) {
  ...
  if (sel === 'shockwave') { swSpeed = extra('sw_speed', 100) / 100; }
  if (sel === 'wormhole') { whTight = extra('wh_tight', 60) / 100; }
  if (sel === 'polar_warp') { pwSegs = Math.max(1, Math.round(extra('pw_segs', 3))); }
  if (sel === 'anamorphic') { anRatio = extra('an_ratio', 100) / 100; }
  if (sel === 'tentacle') { tenArms = Math.max(1, Math.round(extra('ten_arms', 2))); }
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      switch (sel) {
        ...
        // --- Ottica & Cinematica ---
        case 'prism': { const pr_ang = theta + t * 0.3; const pr_push = r * inten * 40 * Math.sin(r * freq * 0.8 + t); sx = x + Math.cos(pr_ang) * pr_push; sy = y + Math.sin(pr_ang) * pr_push * 0.5; break; }
        case 'chromatic_ring': { const cr_push = inten * 60 * Math.sin(r * freq * 3 - t); sx = x + nx * cr_push; sy = y + ny * cr_push; break; }
        case 'anamorphic': { const an_scale = anRatio * (1 + inten * 0.8 * Math.sin(t * 0.5)); sx = cx + (x - cx) * an_scale; sy = y + Math.sin(x / width * Math.PI * freq + t) * inten * 15; break; }
        case 'zoom_blur': { const zb_pull = Math.max(0.2, 1 - inten * 0.5 * Math.abs(Math.sin(t * 0.7))); sx = cx + (x - cx) * zb_pull; sy = cy + (y - cy) * zb_pull; break; }
        case 'light_streak': { const ls_phase = Math.sin(t * 1.2); sx = x + ls_phase * inten * 60 * Math.exp(-Math.abs(y - cy) / (height * 0.25)); sy = y + ls_phase * inten * 25 * Math.exp(-Math.abs(x - cx) / (width * 0.35)); break; }

        // --- Deformazioni ---
        case 'squeeze_x': { const sq_scale = 1 + inten * 1.2 * Math.sin(t * freq * 0.3); sx = cx + (x - cx) * sq_scale; sy = cy + (y - cy) / Math.max(0.3, Math.abs(sq_scale)); break; }
        case 'squeeze_y': { const sqy_scale = 1 + inten * 1.2 * Math.cos(t * freq * 0.3); sy = cy + (y - cy) * sqy_scale; sx = cx + (x - cx) / Math.max(0.3, Math.abs(sqy_scale)); break; }
        case 'shockwave': { const sw_r = Math.sqrt((x - cx) ** 2 + (y - cy) ** 2); const sw_ring = (t * 80 * swSpeed * (freq * 0.12)) % Math.max(width, height); const sw_dist = Math.abs(sw_r - sw_ring); const sw_push = inten * 90 * Math.exp(-sw_dist * sw_dist * 0.0015); sx = x + nx * sw_push; sy = y + ny * sw_push; break; }
        case 'pendulum': { const pend_angle = Math.sin(t * freq * 0.5) * inten * Math.PI * 0.4; const dist_from_top = y / height; sx = x + Math.sin(pend_angle) * dist_from_top * width * 0.5 * inten; sy = y + (1 - Math.cos(pend_angle)) * dist_from_top * height * 0.1 * inten; break; }
        case 'wormhole': { const wh_a = theta + inten * Math.PI * (1 - Math.exp(-r * 3 * whTight)) * Math.sin(t * 0.4); const wh_rr = r * (1 + inten * 0.3 * Math.sin(r * freq * 2 - t)); sx = cx + Math.cos(wh_a) * wh_rr * halfW; sy = cy + Math.sin(wh_a) * wh_rr * halfH; break; }
        case 'earthquake': { const eq_band = Math.floor(y / bk); sx = x + Math.sin(t * 12 + eq_band * 0.7) * inten * 50 * Math.sin(t * 5); sy = y + Math.cos(t * 8 + eq_band * 0.3) * inten * 12; break; }

        // --- Retro & Glitch nuovi ---
        case 'interlace': { const il_even = y % 2 === 0; sx = x + (il_even ? 1 : -1) * Math.sin(t * freq + y * 0.1) * inten * 35; sy = y + (il_even ? 0 : Math.floor(inten * 2)); break; }
        case 'data_mosher': { const dm_bx = Math.floor(x / bk); const dm_by = Math.floor(y / bk); const dm_seed = pseudoRandom(dm_bx + Math.floor(t * 2), dm_by); sx = x + (dm_seed > 0.85 ? (pseudoRandom(dm_bx, dm_by * 7 + t) - 0.5) * inten * width * 0.45 : 0); sy = y + (dm_seed > 0.92 ? inten * bk * Math.sin(t * 3) : 0); break; }

        // --- Acqua nuovi ---
        case 'ripple_box': { const rb_gx = Math.floor(x / bk) * bk + bk / 2; const rb_gy = Math.floor(y / bk) * bk + bk / 2; const rb_dist = Math.sqrt((rb_gx - cx) ** 2 + (rb_gy - cy) ** 2); const rb_phase = Math.sin(rb_dist * 0.05 - t * freq * 0.5) * inten * 35; sx = x + rb_phase * Math.sign(x - cx + 0.01); sy = y + rb_phase * Math.sign(y - cy + 0.01); break; }
        case 'caustic_light': { const ca1 = Math.sin(x * 0.013 + t * 1.1) * Math.cos(y * 0.017 - t * 0.7); const ca2 = Math.sin(x * 0.031 - t * 0.5) * Math.sin(y * 0.021 + t); sx = x + (ca1 + ca2) * inten * 40 * freq * 0.12; sy = y + (ca1 - ca2) * inten * 30 * freq * 0.12; break; }

        // --- Spazio nuovi ---
        case 'polar_warp': { const pw_r = r * (1 + inten * 0.7 * Math.sin(theta * pwSegs + t)); sx = cx + Math.cos(theta) * pw_r * halfW; sy = cy + Math.sin(theta) * pw_r * halfH; break; }
        case 'rose_wave': { const rw_k = Math.max(1, Math.floor(freq)); const rw_rose = Math.cos(rw_k * theta + t) * inten * 0.5; sx = cx + Math.cos(theta) * (r + rw_rose) * halfW; sy = cy + Math.sin(theta) * (r + rw_rose) * halfH; break; }
        case 'lissajous': { const li_a = freq; const li_b = Math.max(1, Math.floor(freq * 1.5)); sx = x + Math.sin(li_a * theta + t) * inten * 70; sy = y + Math.cos(li_b * theta + t * 0.8) * inten * 60; break; }

        // --- Simmetrie nuove ---
        case 'mirror_diag': { const md_sum = x + y; const md_mid = (width + height) / 2; sx = md_sum > md_mid ? width - y + Math.sin(t) * inten * 20 : x + Math.sin(t) * inten * 10; sy = md_sum > md_mid ? height - x + Math.cos(t) * inten * 20 : y + Math.cos(t) * inten * 10; break; }

        // --- Speciali nuovi ---
        case 'tentacle': { const ten_phase = x / width * Math.PI * freq * tenArms + t; sx = x + Math.sin(ten_phase) * inten * 80 * (y / height); sy = y + Math.abs(Math.sin(ten_phase * 0.5)) * inten * 22; break; }
        case 'smoke': { const sm_drift = Math.sin(x * 0.022 + t * 1.5) * Math.cos(x * 0.055 - t) + Math.sin(x * 0.07 + t * 0.7); const sm_rise = 1 - y / height; sx = x + sm_drift * inten * 30 * sm_rise; sy = y - sm_rise * inten * 40 + Math.sin(x * 0.1 + t) * inten * 10; break; }
        case 'coral': { const co_branch = Math.sin(x * freq * 0.055 + t) * Math.cos(y * freq * 0.035 + t * 0.7); const co_depth = y / height; sx = x + co_branch * inten * 45 * co_depth; sy = y + Math.abs(Math.cos(x * freq * 0.035 + t * 0.5)) * inten * 22 * co_depth; break; }
      }
...
function initUI() {
  ...
  ['intensity', 'freq', 'block', 'speed', 'centerX', 'centerY', 'radius', 'feather'].forEach(id => {
    const el = document.getElementById(id);
    if (!el) return;
    el.addEventListener('input', e => {
      state.params[id] = parseFloat(e.target.value);
      if(document.getElementById(`val-${id}`)) {
        const withUnit = ['radius', 'feather'].includes(id) ? `${e.target.value}%` : e.target.value;
  ...
  document.getElementById('btn-reset').addEventListener('click', () => {
    ...
    ['intensity', 'freq', 'block', 'speed', 'centerX', 'centerY', 'radius', 'feather'].forEach(k => {
      const input = document.getElementById(k);
      if(input) input.value = state.params[k];
      const valLabel = document.getElementById(`val-${k}`);
      if (valLabel) {
        valLabel.innerText = ['radius', 'feather'].includes(k) ? `${state.params[k]}%` : state.params[k];
      }
    });
